...
class panglo_parser:
    cls_determiners = ['un', 'due', 'tri', 'for', 'dis', 'dat', 'de', 'sir']
    cls_TAM_markers = ['du', 'ha', 'wil']
    cls_auxiliary_and_modal_verbs = ['du', 'wil', 'ha', 'pote', 'voli', 'mus', 'sabe', 'cing', 'ples']
    cls_personal_pronouns = ['mi', 'yu', 'ho', 'wi', 'yuol', 'dey']
    def __init__(self):
    ...
    def tag_DP(self, tokens):
        ...
        For example, "de frende" would be tagged as "(DP (D de) (N frende))".
        """
    ...
    def tag_PRP(self, tokens):
        ...
        For example, "mi" would be tagged as "(PRP mi)", while "mi se frende" would be tagged as "(NP (PRP$ mi se) (N frende))".
        """
        noun_phrase = Phrase('NP')
        if len(tokens) > 1:
            if tokens[1].lower() == 'se': # Possessive pronoun as a determiner, like "mi se frende" -> "my friend"
                noun_phrase.pos_word_pairs.append(('PRP$', tokens[0] + ' se'))
    ...
    def parse_into_syntax_tree(self, tokens):
        """Parse the input tokens in Panglo into a syntax tree in NLTK format."""
        if self.begins_NP(tokens[0]):
            self.tag_NP(tokens)
        else:
            self.tag_VP(tokens)
